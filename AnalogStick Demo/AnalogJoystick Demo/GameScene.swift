...
class GameScene: SKScene {
    ...
    var joystickStickImageEnabled = true {
        didSet {
            let image = joystickStickImageEnabled ? UIImage(named: "joystick-stick") : nil
    ...
    var joystickSubstrateImageEnabled = true {
        didSet {
            let image = joystickSubstrateImageEnabled ? UIImage(named: "joystick-area") : nil
    ...
    override func didMove(to view: SKView) {
        ...
        rotateJoystick.position = CGPoint(x: size.width - rotateJoystick.diameter, y: rotateJoystick.diameter)
		...
		moveJoystick.on(.move) { [unowned self] joystick in
			guard let appleNode else { return }
			let speed = CGFloat(5)

			appleNode.position = CGPoint(x: appleNode.position.x + (joystick.velocity.x * speed), y: appleNode.position.y + (joystick.velocity.y * speed))
		}
        ...
        let joystickSizeLabel = SKLabelNode(text: "Joystick size:")
